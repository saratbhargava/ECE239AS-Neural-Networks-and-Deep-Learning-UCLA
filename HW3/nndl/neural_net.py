...
class TwoLayerNet(object):
  ...
  def __init__(self, input_size, hidden_size, output_size, std=1e-4):
    ...
    """
    # Separate weights and biases because we regularize weights but not biases
    self.params = {}
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    X_hidden = np.dot(X, W1.T) + b1
    X_hidden_relu = np.maximum(X_hidden, 0)
    scores = np.dot(X_hidden_relu, W2.T) + b2
    ...
    loss = loss - np.sum(np.choose(y, scores.T))
    loss = loss/N
    ...
    scores_rem_max = scores-scores_max[:, np.newaxis]
    Y_pred = np.exp(scores_rem_max)/(np.sum(np.exp(scores_rem_max), axis=1))[:, np.newaxis]
    Y_true = np.zeros_like(Y_pred)
    Y_true[np.arange(N), y] = 1
    g_a = (Y_pred-Y_true)/N
    grads['W2'] = np.dot(g_a.T, X_hidden_relu) + reg*W2
    grads['b2'] = np.sum(g_a, axis=0)
    g_b = np.dot(g_a, W2)
    g_c = (X_hidden > 0)*g_b
    grads['W1'] = np.dot(g_c.T, X) + reg*W1
    grads['b1'] = np.sum(g_c, axis=0)
  ...
  def predict(self, X):
    ...
    X_hidden = np.dot(X, self.params['W1'].T) + self.params['b1']
    X_hidden_relu = np.maximum(X_hidden, 0)
    scores = np.dot(X_hidden_relu, self.params['W2'].T) + self.params['b2']
